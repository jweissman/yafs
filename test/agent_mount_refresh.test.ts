...
import { fakeExchangeModel, waitForRun, manifest, sleep } from './agent_test_helpers'

test('an operator mount refresh keeps run history durable and still picks up a changed prompt', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-agents-refresh-')),
    modelFor: () => fakeExchangeModel('ok', []) })
  const client = await YashClient.connect(server.address())
  await client.exec(`printf '${manifest({ reviewer: 'v1 prompt' })}' > .yafsmeta`)
  ...
  await client.exec(`printf '${manifest({ reviewer: 'v2 prompt' })}' > .yafsmeta`)
  await client.exec('mount refresh .yafsmeta reviewer')
  expect(await client.exec('cat agents/reviewer/prompt.md')).toBe('v2 prompt')
  expect(await client.exec(`cat agents/reviewer/runs/${runId}/status.json`)).toContain('complete')
...
test('a ctl-triggered run leaves an audit entry naming the persona and run, not just the mount', async () => {
  ...
  const server = await YafsServer.start({ dataDir: directory, modelFor: () => fakeExchangeModel('ok', []) })
  const client = await YashClient.connect(server.address())
...
async function lastDetail(directory: string, runId: string): Promise<string | undefined> {
