...
test('an unrecoverable command error destroys the socket instead of hanging it', () => {
  ...
  expect(socket.destroyed).toBe(true); expect(logged).toHaveBeenCalledWith('Unhandled command error:', expect.any(Error))
  logged.mockRestore()
})
...
function fakeServices() {
  ...
  return { store, journal: {} as never, mounts: {} as never, traces: {} as never,
    cache: {} as never, desired: {} as never }
}

function fakeSocket() {
  const socket = new EventEmitter() as EventEmitter & { destroyed: boolean, destroy(): void, setEncoding(): void }
  socket.destroyed = false; socket.destroy = () => { socket.destroyed = true }; socket.setEncoding = () => undefined
  return socket
}
