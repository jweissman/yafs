...
test('a due daemon refresh publishes the next complete GitHub snapshot', async () => {
  const pulls = [pull()]; const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-refresh-')),
    now: () => Date.now() + 120_000,
    providers: new ProviderRegistry(new GitHubCollectionSource({ pulls: async () => pulls })) })
  const client = await YashClient.connect(server.address()); await client.exec(`printf '${scheduledManifest()}' > .yafsmeta`)
  await client.exec('mount activate .yafsmeta'); pulls[0] = { ...pull(), diff: 'new diff' }; await server.refreshDue()
...
function scheduledManifest() { return manifest().replace('capabilities:', 'refresh: {interval: 1m}, capabilities:') }
