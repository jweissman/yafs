...
test('a slow background delivery does not block other clients while it is in flight', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-stream-')) })
  const writer = await YashClient.connect(server.address()); await writer.exec('mkdir stream')
  const delivery = deliver(server, '/home/root/stream/output.txt', ['one', 'two', 'three'], 200)
  const reader = await YashClient.connect(server.address())
  const start = Date.now(); expect(await reader.exec('echo still responsive')).toBe('still responsive')
  expect(Date.now() - start).toBeLessThan(100)
  await delivery
  expect(await reader.exec('cat stream/output.txt')).toBe('onetwothree')
  await writer.close(); await reader.close(); await server.close()
})

test('a client can read partial content mid-delivery, not just the final result', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-stream-partial-')) })
  const writer = await YashClient.connect(server.address()); await writer.exec('mkdir stream')
  const delivery = deliver(server, '/home/root/stream/output.txt', ['first-', 'second-', 'third'], 150)
  await sleep(180)
  const reader = await YashClient.connect(server.address())
...
test('a registered ctl handler intercepts real shell redirection, not just a raw write RPC', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-ctl-')) })
  ...
  server.registerCtl('/home/root/stream/ctl' as AbsolutePath, restart(server, path, ['one-', 'two-', 'three'], 60))
  const start = Date.now(); await client.exec('printf restart > stream/ctl')
...
test('unregistering a ctl handler restores ordinary write behavior for that path', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-ctl-unregister-')) })
  const client = await YashClient.connect(server.address()); await client.exec('mkdir stream')
  const ctlPath = '/home/root/stream/ctl' as AbsolutePath
  server.registerCtl(ctlPath, () => {}); server.unregisterCtl(ctlPath)
...
async function deliver(server: YafsServer, path: AbsolutePath, chunks: string[], intervalMs: number) {
  ...
  for (const chunk of chunks) {
    await sleep(intervalMs); content += chunk
    await server.commitBackground([{ type: 'write', path, content, at: new Date().toISOString() }])
  }
}
