...
test("workspace operations return typed VFS values without shell parsing", async () => {
  ...
  expect(await yafs.operations.invoke({ name: "list", path: "work" })).toEqual({
    kind: "list", path: "/home/root/work", entries: ["brief.md"],
  });
  expect(await yafs.operations.invoke({ name: "read", path: "work/brief.md" })).toEqual({
    kind: "read", path: "/home/root/work/brief.md", text: "brief",
  });
  expect((await yafs.operations.invoke({ name: "inspect", path: "work/brief.md" })).kind).toBe("inspect");
});

test("a typed workspace operation returns a structured failure", () => {
  const result = new Yafs().planOperation({ name: "read", path: "missing" }).result;
...
test("typed evidence operations preserve captured bytes across source changes", async () => {
  ...
  const rejected = await yafs.planOperationAsync({
    name: "capture", source: "source", artifact: "artifacts/rejected", limit: 0,
  });
  ...
  const oversized = await yafs.planOperationAsync({
    name: "restore", artifact: "artifacts/oversized", destination: "too-many",
  });
...
function oversizedTrace() {
  ...
  const entries = Array.from({ length: 10001 }, (_, index) => ({
    path: String(index), digest,
  }));
  return JSON.stringify({
    kind: "yafs-trace", version: 1, sourcePath: "/source",
    capturedAt: "2026-01-01T00:00:00.000Z", entries,
  });
}
