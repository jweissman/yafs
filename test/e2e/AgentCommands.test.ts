...
test("agent target resolves a bare persona name to its full path", async () => {
  const { server, client } = await startedHostConfigServer(
    "yafs-agent-target-",
    manifest({ reviewer: "You are a reviewer" }),
  );
  await client.exec("plugins apply");
  expect(await client.exec("agent target reviewer")).toBe(
    "/home/root/agents/reviewer",
  );
  await client.close();
  await server.close();
});
