...
test('a GitHub collection becomes an immutable, attributable review snapshot', async () => {
  ...
  expect((await yafs.executeAsync('mount refresh .yafsmeta')).stdout).toBe('review refreshed')
  expect(yafs.exec('cat reviews/pulls/42/diff.patch')).toBe('diff --git a/a b/a')
  expect(JSON.parse(yafs.exec('cat reviews/pulls/42/metadata.json'))).toMatchObject({ number: 42 })
  expect(JSON.parse(yafs.exec('inspect reviews/pulls/42/diff.patch')).origins[0]).toMatchObject({
    provider: 'github', mountId: 'review', revision: expect.stringMatching(/^github:/) })
})
...
test('a declared secret grant selects an authenticated provider source', async () => {
  const publicSource = new GitHubCollectionSource({ pulls: async () => [pull('public')] })
  const privateSource = new GitHubCollectionSource({ pulls: async () => [pull('private')] })
  const yafs = configuredYafs(new ProviderRegistry(publicSource, privateSource))
  yafs.store.write('/home/root/.yafsmeta', githubManifest().replace('[network.github-api]', '[network.github-api, secret.github-token]'))
...
function fakeSource() { return new GitHubCollectionSource(fakeClient()) }
...
function pull(diff: string) {
  return { number: 42, title: 'Improve resolver', updatedAt: '2026-08-03T00:00:00Z', headSha: 'abc123', diff }
}
