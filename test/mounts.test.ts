...
test('a validated manifest activates a read-only fixture mount with provenance', () => {
  ...
  verifyFixture(yafs)
  expect(JSON.parse(yafs.exec('inspect fixture/hello.txt')).origins[0]).toMatchObject({
    kind: 'provider', mountId: 'demo', provider: 'fixture' })
})
...
test('mount activation persists state, audit, and fixture content across restart', async () => {
  ...
  await access(join(directory, 'mounts.json')); await access(join(directory, 'audit.ndjson'))
  expect(await readFile(join(directory, 'audit.ndjson'), 'utf8')).toContain('"afterRevision":"fixture:')
  ...
  await restored.exec('mount unmount demo'); await restored.exec('mount activate .yafsmeta')
  await restored.close(); await restarted.close()
  expect(auditSequences(await readFile(join(directory, 'audit.ndjson'), 'utf8'))).toEqual([1, 2, 3])
})
...
test('fixture snapshots participate in links, unions, and provenance', () => {
  const yafs = new Yafs(); yafs.store.write('/home/root/.yafsmeta', fixtureManifest())
  yafs.exec('mount activate .yafsmeta'); yafs.exec('mkdir notes')
  yafs.exec('echo local > notes/alice.md'); yafs.exec('ln -s fixture/hello.txt latest')
  yafs.exec('union review notes fixture')
  expect(yafs.exec('cat latest')).toBe('hello')
  expect(yafs.exec('ls review')).toBe('alice.md\nhello.txt')
  expect(JSON.parse(yafs.exec('inspect review/hello.txt')).origins[0]).toMatchObject({
    kind: 'provider', mountId: 'demo', provider: 'fixture' })
})

test('a mount may not replace an existing local node', () => {
  const yafs = new Yafs(); yafs.exec('mkdir fixture')
  yafs.store.write('/home/root/.yafsmeta', fixtureManifest())
  expect(yafs.execute('mount activate .yafsmeta').stderr).toBe('Mount path already exists: /home/root/fixture')
})
...
test('nested snapshot files survive replay and disappear after durable unmount', async () => {
  ...
  const restarted = await YafsServer.start({ dataDir: directory })
  const restored = await YashClient.connect(restarted.address())
  expect(await restored.exec('cat /home/root/fixture/nested/hello.txt')).toBe('hello')
  await restored.exec('mount unmount demo'); await restored.close(); await restarted.close()
  const afterUnmount = await YafsServer.start({ dataDir: directory })
  const verified = await YashClient.connect(afterUnmount.address())
  expect((await verified.execute('cat /home/root/fixture/nested/hello.txt')).error?.code).toBe('not_found')
  await verified.close(); await afterUnmount.close()
})
