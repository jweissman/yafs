...
test('a yash client talks to a persistent server', async () => {
  ...
  expect(await client.exec('mkdir notes')).toBe(''); expect(await client.exec('touch notes/note.md')).toBe('')
...
test('a yash client rejects unsupported and failed protocol replies', async () => {
  ...
  await expect(client.exec('pwd')).rejects.toThrow('Unsupported protocol version')
  await client.close(); await close(server)
})
...
test('a malformed request closes only that client connection', async () => {
  ...
  const malformed = createConnection(server.address())
  await waitForMalformedClose(malformed)
  const client = await YashClient.connect(server.address()); expect(await client.exec('pwd')).toBe('/home/root')
  await client.close(); await server.close()
})
...
test('journal recovers torn tails and rejects earlier corruption', async () => {
  ...
  const client = await YashClient.connect(server.address())
  await client.exec('touch durable'); await client.close(); await server.close(); await appendFile(wal, '{"torn"')
  const recovered = await YafsServer.start({ walPath: wal })
  const restored = await YashClient.connect(recovered.address())
  expect(await restored.exec('stat durable')).toBe('file'); await restored.close(); await recovered.close()
  await appendFile(wal, 'bad record\n'); await expect(YafsServer.start({ walPath: wal })).rejects.toThrow('Corrupt journal record')
})
...
test('journal restores snapshots with stale pre-compaction WAL records', async () => {
  ...
  const client = await YashClient.connect(server.address())
  for (let index = 0; index < 31; index++) await client.exec(`touch item-${index}`)
  const staleWal = await readFile(wal, 'utf8')
  await client.exec('touch item-31'); await client.close(); await server.close()
  await writeFile(wal, staleWal)
  const restarted = await YafsServer.start({ dataDir: directory })
  const reconnected = await YashClient.connect(restarted.address())
  expect(await reconnected.exec('stat item-31')).toBe('file'); await reconnected.close(); await restarted.close()
})

function waitForMalformedClose(socket: ReturnType<typeof createConnection>) {
...
function address(server: ReturnType<typeof createServer>) {
  ...
  if (!value || typeof value === 'string') throw new Error('Server is not listening')
