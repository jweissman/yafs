...
test('the background refresh timer itself, not just a manually-triggered refresh, picks up due mounts', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-refresh-timer-')),
    now: () => Date.now() + 120_000, refreshIntervalMs: 5,
    providers: new ProviderRegistry(new GitHubCollectionSource(countingClient())) })
  const client = await YashClient.connect(server.address())
  await client.exec(`printf '${scheduledManifest()}' > .yafsmeta`); await client.exec('mount activate .yafsmeta')
...
async function waitForChange(client: YashClient, initial: string) {
  for (let attempt = 0; attempt < 100; attempt++) {
    if (await client.exec('cat reviews/pulls/42/metadata.json') !== initial) return
...
function countingClient() {
  let calls = 0
  return { pulls: async () => { calls++; return [pull(calls)] } }
}
function pull(revision = 1) {
  return { number: 42, title: `Review ${revision}`, updatedAt: '2026-08-03T00:00:00Z', headSha: 'abc123', diff: 'diff --git' }
}
