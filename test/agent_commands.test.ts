...
test('agent pseudobinaries send, inspect, and cancel a run without ctl JSON', async () => {
  ...
  expect(accepted).toMatch(/^accepted: agents\/reviewer -> \/home\/root\/agents\/reviewer\/runs\/[\w-]+$/)
  const run = accepted.split(' -> ')[1]; const runId = run.split('/').pop()!
  await waitForStatus(client, 'agents/reviewer/runs', status => status.state === 'running')
  expect(JSON.parse(await client.exec(`agent status ${run}`)).state).toBe('running')
  expect(await client.exec(`agent cancel agents/reviewer ${runId}`)).toBe(`cancelling: agents/reviewer ${runId}`)
...
test('agent send resolves a bare persona name from anywhere, not just its own directory', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-agent-cwd-')),
  ...
  await client.exec('mkdir elsewhere'); await client.exec('cd elsewhere')
  const accepted = await client.exec('agent send reviewer "hi"')
  expect(accepted).toContain('accepted: reviewer -> /home/root/agents/reviewer/runs/')
  await expect(client.exec('agent send nope "hi"')).rejects.toThrow('No such persona: nope')
  await client.close(); await server.close()
})

test('agent send rejects an ambiguous bare persona name shared by two plugins', async () => {
  const server = await YafsServer.start({ dataDir: await mkdtemp(join(tmpdir(), 'yafs-agent-ambiguous-')),
    modelFor: () => fakeMessageModel([]) })
  const client = await YashClient.connect(server.address())
  await client.exec(`printf '${manifest({ reviewer: 'prompt' })}' > .yafsmeta`); await client.exec('mount activate .yafsmeta')
  const second = manifest({ reviewer: 'prompt' }).replace('id: reviewer', 'id: second').replace('path: agents', 'path: agents2')
  await client.exec(`printf '${second}' > .yafsmeta2`); await client.exec('mount activate .yafsmeta2 second')
  await expect(client.exec('agent send reviewer "hi"')).rejects.toThrow('Ambiguous persona reviewer')
  expect(await client.exec('agent send agents2/reviewer "hi"')).toContain('accepted:')
  await client.close(); await server.close()
})
