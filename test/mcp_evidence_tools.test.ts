...
test("evidence tools map fixed MCP input to typed operations", () => {
  ...
  expect(evidenceOperation("yafs.capture", {
    source: "/source", artifact: "/artifacts/one",
  })).toEqual({ name: "capture", source: "/source", artifact: "/artifacts/one" });
  expect(evidenceOperation("yafs.restore", {
    artifact: "/artifacts/one", destination: "/restored",
  })).toEqual({ name: "restore", artifact: "/artifacts/one", destination: "/restored" });
  expect(evidenceOperation("yafs.unknown", {})).toBeUndefined();
});

test("evidence tools reject host and relative paths", () => {
  expect(() => evidenceOperation("yafs.capture", {
    source: "relative", artifact: "/artifacts/one",
  })).toThrow("absolute Yafs path");
  expect(() => evidenceOperation("yafs.restore", {
    artifact: "/artifacts/one", destination: 1,
  })).toThrow("absolute Yafs path");
});
