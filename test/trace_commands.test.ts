...
test('trace captures a provider subtree and reify preserves it after refresh', async () => {
  ...
  yafs.store.write('/home/root/.yafsmeta', manifest()); await yafs.executeAsync('mount activate .yafsmeta')
  yafs.exec('mkdir notes'); await yafs.executeAsync('trace reviews/pulls/42 notes/42')
  const trace = JSON.parse(yafs.exec('cat notes/42/trace.json'))
  expect(trace).toMatchObject({ sourcePath: '/home/root/reviews/pulls/42', origin: { provider: 'github' },
    resourceReference: { kind: 'github-pr', repository: 'acme/widget', number: 42, headSha: 'abc123' } })
  pulls.length = 0; await yafs.executeAsync('mount refresh .yafsmeta')
  await yafs.executeAsync('reify notes/42 restored'); expect(yafs.exec('cat restored/diff.patch')).toBe('diff --git')
})

test('trace creates a durable artifact only for a directory and reify rejects existing output', async () => {
  const yafs = new Yafs(); yafs.exec('mkdir notes'); yafs.exec('echo text > source.txt')
  expect((await yafs.executeAsync('trace source.txt notes/source')).stderr).toContain('must be a directory')
  await yafs.executeAsync('trace notes notes/copy')
  expect((await yafs.executeAsync('reify notes/copy notes')).stderr).toContain('already exists')
})

test('blobs gc is an explicit control command', async () => {
  const yafs = new Yafs(); const result = await yafs.executeAsync('blobs gc')
...
test('a failed trace plan leaves blobs unretained for explicit collection', async () => {
  const yafs = new Yafs(); yafs.exec('mkdir source'); yafs.exec('echo captured > source/a.txt')
  expect((await yafs.executeAsync('trace source missing/artifact')).error).toBeDefined()
  expect(JSON.parse((await yafs.executeAsync('blobs gc')).stdout).reclaimed).toHaveLength(1)
})
