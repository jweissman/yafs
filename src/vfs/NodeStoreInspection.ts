...
export class NodeStoreInspection {
  ...
  read(path: AbsolutePath): string {
    const node = this.resolver.get(path); if (!node) throw new Error(`No such file: ${path}`)
    if (node.dir) throw new Error(`Is a directory: ${path}`); return node.content || ''
  }
  readlink(path: AbsolutePath): string {
    const node = this.resolver.get(path, false); if (!node) throw new Error(`No such file: ${path}`)
    if (!node.symlinkTarget) throw new Error(`Not a symbolic link: ${path}`); return node.symlinkTarget
  }
  type(path: AbsolutePath, follow = true): 'file' | 'directory' | 'symlink' {
    const node = this.resolver.get(path, follow); if (!node) throw new Error(`No such file: ${path}`)
    return node.symlinkTarget ? 'symlink' : node.dir ? 'directory' : 'file'
  }
  list(path: AbsolutePath): string[] {
    const node = this.resolver.get(path); if (!node) throw new Error(`No such directory: ${path}`)
  ...
  private childOrigins(node: FSNode, parts: string[], path: AbsolutePath) {
    const child = node.children?.find(item => item.name === parts[0]); if (!child) throw new Error(`No such file: ${path}`)
    return parts.length === 1 ? [this.resolver.pathOf(child)] : this.nextOrigins(child, parts, path)
  }
