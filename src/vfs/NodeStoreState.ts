...
export class NodeStoreState {
  ...
  createNode(name: string, dir: boolean, parent: FSNode, at = this.clock.now()) {
    const node = this.node(name, dir, parent, at)
    this.index(node); this.attach(parent, node)
    return node
  }

  private node(name: string, dir: boolean, parent: FSNode, at: Date): FSNode {
    return { name, dir, children: dir ? [] : undefined, parent, createdAt: at, modifiedAt: at }
  }
  private index(node: FSNode) { this.nodes[this.nextInode++] = node }
  private attach(parent: FSNode, node: FSNode) { parent.children ||= []; parent.children.push(node) }
}
