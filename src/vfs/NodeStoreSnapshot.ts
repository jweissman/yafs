...
export class NodeStoreSnapshot {
  constructor(
    ...
    private readonly mutate: NodeStoreMutation,
  ) {}
  ...
  private copy() {
    const state = new NodeStoreState(this.state.clock);
  ...
  restore(snapshot: VfsSnapshot) {
    this.state.nodes = { 1: this.restoreNode(snapshot.root, undefined) };
    this.state.nextInode = 2;
  ...
  private nodeMetadata(node: FSNode) {
    return {
      providerOrigin: node.providerOrigin,
  ...
  private index(node: FSNode) {
    node.children?.forEach((child) => {
      this.state.nodes[this.state.nextInode++] = child;
      this.index(child);
    });
  }
}
