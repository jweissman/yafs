...
function fixtureSnapshot(record: MountRecord, snapshots: SnapshotMaterializer, current?: PreparedMountRecord) {
  const config = record.config as FixtureConfig
  const fresh = FixtureProvider.from(config).entries()
  return snapshots.prepare(record, carryForward(fresh, current, path => Boolean(config.streams?.[path])))
}
