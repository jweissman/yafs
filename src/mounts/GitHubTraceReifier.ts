...
import { GitHubPullFetcher, GitHubResourceReference, pullFile } from './GitHubCollectionSource'
...
export class GitHubTraceReifier implements TraceReifier {
  ...
  async reify(trace: Trace, digest: string): Promise<Uint8Array | undefined> {
    const entry = trace.entries.find(candidate => candidate.digest === digest); const reference = this.reference(trace)
  ...
  private reference(trace: Trace): GitHubResourceReference | undefined {
    const reference = trace.resourceReference as GitHubResourceReference | undefined
