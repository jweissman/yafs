...
export type GitHubPullFetcher = { pull(repository: string, number: number): Promise<GitHubPull> }
...
export type GitHubResourceReference = { kind: 'github-pr', repository: string, number: number, headSha: string }
...
export class GitHubCollectionSource {
  ...
  private pullEntries(pull: GitHubPull): [string, string][] {
    ...
    return [[`${root}/diff.patch`, pullFile(pull, 'diff.patch')!], [`${root}/metadata.json`, pullFile(pull, 'metadata.json')!]]
  }
  private references(config: GitHubConfig, pulls: GitHubPull[]) {
    return Object.fromEntries(pulls.map(pull => [`pulls/${pull.number}`, this.reference(config, pull)]))
  }
  private reference(config: GitHubConfig, pull: GitHubPull): GitHubResourceReference {
    return { kind: 'github-pr', repository: config.repository, number: pull.number, headSha: pull.headSha }
  }
...
export function pullFile(pull: GitHubPull, name: string): string | undefined {
  if (name === 'diff.patch') return pull.diff
  if (name === 'metadata.json') return JSON.stringify(pullMetadata(pull))
  return undefined
}
