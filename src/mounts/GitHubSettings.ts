...
export function githubSettings(environment = process.env): GitHubSettings {
  ...
  if (!apiUrl.startsWith('https://')) throw new Error('YAFS_GITHUB_API_URL must use https')
  return { apiUrl: apiUrl.replace(/\/$/, ''), token: environment.YAFS_GITHUB_TOKEN }
}

function resolvedApiUrl(environment: NodeJS.ProcessEnv) {
  return environment.YAFS_GITHUB_API_URL || apiUrlForHost(environment.YAFS_GITHUB_HOST || 'github.com')
}
