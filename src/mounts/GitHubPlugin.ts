...
export function githubDefinition(sources: Sources): ProviderDefinition {
  return {
    name: "github",
    capabilities: () => capabilities(sources.authenticatedGithub),
...
function capabilities(authenticatedGithub?: GitHubCollectionSource) {
  return [
    "network.github-api",
    ...(authenticatedGithub ? ["secret.github-token"] : []),
  ];
}
