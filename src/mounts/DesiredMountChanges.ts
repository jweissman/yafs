...
export class DesiredMountChanges {
  ...
  private assertUnique(declared: ManifestMount[]) {
    if (new Set(declared.map((item) => item.id)).size !== declared.length) {
      throw new Error("Duplicate desired mount id");
    }
  }

  private declaredChanges(
    declared: ManifestMount[],
    active: PreparedMountRecord[],
  ): Change[] {
  ...
  private change(
    item: ManifestMount,
    active: PreparedMountRecord | undefined,
  ): Change[] {
    if (!active) {
  ...
  private matches(item: ManifestMount, active: PreparedMountRecord) {
    return (
      JSON.stringify(this.activeShape(active)) ===
      JSON.stringify(this.declaredShape(item))
    );
  }
  ...
  private declaredShape(item: ManifestMount) {
    return [
      PathResolver.resolve(item.path, this.root),
      item.provider,
      item.config,
      item.capabilities,
