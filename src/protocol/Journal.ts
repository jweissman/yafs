...
import { appendAndSync, acquireLock, prepareJournal, removeIfPresent, truncateAndSync } from './JournalStorage'
...
export class Journal {
  ...
  static async open(walPath: string, store: NodeStore, replay?: JournalReplayer): Promise<Journal> {
    const journal = await Journal.lock(walPath)
    return journal.restore(store, replay)
  }

  private static async lock(walPath: string) {
    await prepareJournal(walPath); const lockPath = `${walPath}.lock`; await acquireLock(lockPath)
    return new Journal(walPath, lockPath, 0)
  }

  private async restore(store: NodeStore, replay?: JournalReplayer): Promise<Journal> {
    try { this.sequence = await this.recoveredSequence(store, replay); return this }
  ...
  async close() { await removeIfPresent(this.lockPath) }

  private record(operation: JournalRecord['operation'], index: number): JournalRecord {
