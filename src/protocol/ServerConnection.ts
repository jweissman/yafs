...
import { attachLines, isCacheRequest, isWriteRequest, persistenceFailure, requestOrReject, respond,
...
export class ServerConnection {
  ...
  constructor(private readonly services: Services, private readonly background: () => BackgroundDrivers) {}

  attach(socket: Socket) { this.observe(socket); this.attachSession(socket, this.session()) }
  closeSockets() { this.sockets.forEach(socket => socket.destroy()) }
  enqueueWork(work: () => Promise<void>) { this.queue = this.queue.then(work); return this.queue }

  session() {
    const s = this.services
    return new Yafs({ store: s.store, mounts: s.mounts, traces: s.traces, cache: s.cache, desired: s.desired })
  }
  ...
  private enqueue(session: Yafs, line: string, socket: Socket) {
    ...
    this.queue = this.queue.then(run).catch(error => this.abort(error, socket))
  }

  abort(error: unknown, socket: Socket) { console.error('Unhandled command error:', error); socket.destroy() }
  ...
  async commit(session: Yafs, operations: VfsOperation[]) {
    await this.services.journal.commit(operations); session.apply(operations); syncAll(this.background())
    try { await this.services.journal.compact(this.services.store) }
    catch (error) { console.error('Journal compaction failed:', error) }
  }
}
