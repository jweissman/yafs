...
import {
  NodeType, WorkspaceOperation, WorkspaceValue,
...
export function workspaceLiteracyCommands(): BuiltinCommand[] {
  return [command("tree", tree), command("find", find), command("test", test), command("diff", diff)];
}
...
function command(
  ...
  return {
    name, synopsis: name, access: "read",
...
function tree(args: string[]): WorkspaceOperation {
  ...
  return {
    name: "tree", path: required(paths, 0), depth: number(depth),
    limit: number(limit),
  };
}
...
function find(args: string[]): WorkspaceOperation {
  ...
  return {
    name: "find", path: required(paths, 0), pattern,
    type: nodeType(type), limit: number(limit),
  };
}

function test(args: string[]): WorkspaceOperation {
  return { name: "test", predicate: required(args, 0) as "-e" | "-f" | "-d" | "-L", path: required(args, 1) };
}

function diff(args: string[]): WorkspaceOperation {
  const [limit, paths] = option(args, "--limit");
  return {
    name: "diff", left: required(paths, 0), right: required(paths, 1),
    limit: number(limit),
  };
}

function render(value: WorkspaceValue) {
  return value.kind === "tree" ? value.entries.map((entry) => entry.path).join("\n")
    : value.kind === "find" ? value.paths.join("\n")
      : value.kind === "test" ? String(value.value)
        : value.kind === "diff" ? value.changes.map(format).join("\n") : "";
}
