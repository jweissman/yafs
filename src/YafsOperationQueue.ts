...
export class YafsOperationQueue {
  ...
  reset() { this.operations = []; this.afterApply = [] }
  all() { return this.operations }
  count() { return { operations: this.operations.length, effects: this.afterApply.length } }
  restore(count: { operations: number, effects: number }) {
    this.operations.length = count.operations; this.afterApply.length = count.effects
  }
  ...
  afterCommit(effect: () => void) { this.afterApply.push(effect) }

  apply(operations = this.operations) {
    operations.forEach(operation => this.applyOperation(operation)); this.runAfterApply()
  }

  private runAfterApply() { this.afterApply.forEach(effect => effect()); this.afterApply = [] }
