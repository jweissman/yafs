...
class AgentCommand {
  readonly name = "agent";
  readonly synopsis =
    "agent send PERSONA [--context PATH] MESSAGE | status RUN | cancel PERSONA RUN";
  ...
  private request(context: CommandContext, args: string[]) {
    if (args[2] !== "--context") {
      return { message: context.required(this.name, args, 2) };
    }
    const path = context.required(this.name, args, 3);
    const message = context.required(this.name, args, 4);
