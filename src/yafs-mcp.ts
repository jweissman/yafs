...
const client = await YashClient.connect({ host: process.env.YAFS_HOST || '127.0.0.1',
  port: Number(process.env.YAFS_PORT || 7337) })
const server = new McpServer(client)
const lines = createInterface({ input: stdin, crlfDelay: Infinity })

try { for await (const line of lines) await respond(line) }
finally { await client.close() }

async function respond(line: string) {
  try { write(await server.receive(JSON.parse(line))) }
  catch (error) { write(failure(error)) }
}
