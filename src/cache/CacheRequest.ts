...
function validOperation(request: Partial<CacheRequest>) {
  if (request.operation === 'gc') return true
  if (request.operation === 'put') return strings(request.key, request.value) && Number.isSafeInteger(request.ttlMs)
  return ['get', 'stat', 'delete'].includes(request.operation || '') && typeof request.key === 'string'
}
