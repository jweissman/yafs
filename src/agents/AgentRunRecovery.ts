...
export async function recoverAgentRuns(
  ...
  for (const record of mounts.mounts()) {
    await recoverRecord(record, runs);
  }
}
...
async function recoverRecord(record: PreparedMountRecord, runs: AgentRunStore) {
  ...
  for (const [path, content] of record.snapshot.entries) {
    await interrupt(record.id, path, content, runs);
  }
}
...
async function interrupt(
  ...
  const id = { mountId, personaName: run.persona, runId: run.id };
  await runs.interrupt(id, interruptedStatus(run.startedAt));
}

function interruptedStatus(startedAt: string) {
  return {
